...
function emptyCollectionTitleTwitterLoad() {
    ...
    if (!target) {
        //The twiiter container node not loaded yet. Wait 500ms and try again
        window.setTimeout(emptyCollectionTitleTwitterLoad, 500);
        return;
    }
...
function replaceCollectionTitleTwitterLoad() {
        ...
        if( collection_decription ){
          ...
          setTimeout(function(){
            var shareURL = chrome.storage.local.get({'link_text_share_url': null}, (res) => {
              var link_text_share_url = res.link_text_share_url;
              if( window.location.href.match('/timelines/[(0-9)]'))
              if( link_text_share_url )
                jQuery('body').append( '<div class="gl-share-button-div" style="position:fixed; right:25px; bottom:75px; background:white; width: 400px; height:80px; border:0px solid rgb(29, 161, 242); border-radius: 16px;box-shadow: rgba(101, 119, 134, 0.2) 0px 0px 15px, rgba(101, 119, 134, 0.15) 0px 0px 3px 1px;""> <a href="' + link_text_share_url + '" class="social-share-btn"> Share this collection</a></div>' );
                //document.querySelector("body").innerHTML += '<div style="position:fixed; right:25px; bottom:75px; background:white; width: 400px; height:80px; border:0px solid rgb(29, 161, 242); border-radius: 16px;box-shadow: rgba(101, 119, 134, 0.2) 0px 0px 15px, rgba(101, 119, 134, 0.15) 0px 0px 3px 1px;""> <a href="' + link_text_share_url + '" class="social-share-btn"> Share this collection</a></div>';
