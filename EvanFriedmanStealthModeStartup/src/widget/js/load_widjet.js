...
function loadJs (tabInfo,linkId ) {
  ...
  script.addEventListener('load', () => {
    loadingInfo(tabInfo,linkId).then((data) => {
      ...
      document.body.onscroll = function () {
        if (data.stats.total_tweets < 30 || count > 2) return;
...
function media_events(data){
  ...
  document.getElementById('fullPageTweets').onclick = () =>document.getElementById('fullPageTweets-view').click();
  ...
  if(data.article_summary && data.collection_url){
   ...
   topic_title = encodeURIComponent(topic_title)
    ...
    chrome.storage.local.set({"link_text_share_url": shareURl}, function () {
